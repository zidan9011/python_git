...
def get_index_info(appname,appversion,last_month_first,this_month_last):
    ...
    need_sql = "SELECT AppName,AppVersion,UpdateDate,BaseLine,PageNumber,UppradeTime,version_num,environment_fir,UpgradeMan,update_reason,project FROM cm_vrms_baseline_cm_baseline_info where AppName like '%"+appname+"%' and AppVersion like '%"+appversion+"%' and UpdateDate <= '"+this_month_last+"' order by UpdateDate desc;" 
...
def cm_bi_index(request):
    ...
    cursor1.execute("SELECT COUNT(AppName) FROM `cm_vrms_baseline_cm_baseline_info` where status_id !=5 and UpdateDate BETWEEN '"+this_month_first+"' and '"+this_month_last+"';")
    ...
    for count_info in count_sys:
        ...
        if "本币交易系统" not in appname.encode("utf8"):
...
def bi_update(request):
    ...
    cursor1 = connection.cursor() 
    cursor1.execute("select project,COUNT(AppName) from cm_vrms_baseline_cm_baseline_info where status_id !=5 and UpdateDate BETWEEN '"+this_month_first+"' and '"+this_month_last+"' GROUP  BY project;")
    count_currentnum = cursor1.fetchall()
    current_num = dict([[val[0],int(val[1])] for val in count_currentnum])
    
    cursor2 = connection.cursor() 
    cursor2.execute("SELECT project,COUNT(id) FROM cm_vrms_baseline_cm_baseline_info where UpdateDate BETWEEN  '"+last_month_first+"' and '"+last_month_last+"' GROUP  BY project;")
...
def get_envi_down_ration(this_month_first,this_month_last,last_month_first,last_month_last):
    ...
    for i,name in enumerate(current_info_join):
        ...
        if last_info_join[i] > 0:
            ...
            down_ratio = "{0:.4}%".format(down_ratio*100)
        out_str += "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>".format(down_type,last_info_join[i],current_info_join[i],down_ratio) 
    return out_str
...
def bi_human(request):
    ...
    cursor1 = connection.cursor() 
    cursor1.execute("select author_name, COUNT(id) from cm_vrms_baseline_cm_baseline_subject_info where status_id !=5 and update_date between '"+this_month_first+"' and '"+this_month_last+"' and environment_fir in ('UAT环境','模拟环境') GROUP BY author_name;")
    count_currentnum = cursor1.fetchall()
